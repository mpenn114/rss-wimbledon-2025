...
def assign_tokens(expected_prize_money:pd.DataFrame, lucky_losers:List[str] = []):
    ...
    overall_expectation_data = pd.concat([seeding_prize_money, expected_prize_money.set_index('player_name')['mean_prize_money']], axis=1).reset_index()
    overall_expectation_data.columns = ['player_name','seeding_based_money','mean_prize_money']

    # Get the alpha distribution
    alpha_grid, optimal_alpha_pdf, _ = calculate_alpha_distribution(overall_expectation_data['mean_prize_money'].to_numpy())

    # Fill in mean prize money with average 1st-v-2nd round loser winnings
    prize_money = define_prize_money()
    overall_expectation_data['seeding_based_money'] = overall_expectation_data['seeding_based_money'].fillna(0.5*(prize_money[0] + prize_money[1]))
    ...
    for _ in tqdm(range(2), desc = 'Assigning Tokens....'):
        ...
        token_df, token_assignment = create_token_assignments(overall_expectation_data.copy(), alpha_grid, optimal_alpha_pdf)
        ...
        optimal_solution = minimize(optimise_strategy, 10*np.ones(256)/256, method='L-BFGS-B',
                                    options={
...
def get_seeding_based_expected_winnings(seeding:pd.Series) -> pd.Series:
    ...
    prize_money = define_prize_money()
    return pd.Series({player:prize_money[round] for player, round in zip(seeding.index,seeding_round_reached )})
...
def load_seedings() -> pd.Series:
    ...
    male_seedings = pd.Series([
        ...
        "Griekspoor T.",
        "Berrettini M."
    ], index=range(1, 33), name="2025 Wimbledon Men's Seeds")
    ...
    womens_seedings = pd.Series([
        ...
        "Krueger A.",
        "Kessler M."
    ], index=range(1, 33), name="2025 Wimbledon Women's Seeds")

    # Flip both series to be name -> seeding
    male_seedings_flipped = pd.Series(male_seedings.index.values, index=male_seedings.values)
    womens_seedings_flipped = pd.Series(womens_seedings.index.values, index=womens_seedings.values)
...
def create_token_assignments(overall_expectation_data:pd.DataFrame, alpha_grid:np.ndarray, optimal_alpha_pdf:np.ndarray) -> Tuple[pd.DataFrame, np.ndarray]:
    """
    Create the token assignments from the random players

    Args:
        overall_expectation_data (pd.Dataframe): The dataframe containing true and seedings-
            based expected winnings
    ...
    token_assignment = assign_player_token_weight(player_expectations,alpha[np.newaxis,:])
...
def calculate_alpha_distribution(players_winnings:np.ndarray,alpha_max:int=1.0, alpha_min:int= -1.0, grid_size:int = 100,
                                 ) ->Tuple[np.ndarray,np.ndarray]:
    """
    Calculate the alpha distribution

    We aim to calculate the distribution of alpha such that every alpha has the same expected prize money
    """
    ...
    total_assigned = assign_player_token_weight(players_winnings_normalised[np.newaxis,:],alpha_grid[:,np.newaxis])
    ...
    def objective(alpha_pdf:np.ndarray):
        ...
        total_tokens_assigned = np.sum(total_normalised_assigned*alpha_pdf[:,np.newaxis], axis=0)
        total_winnings = np.sum(
            total_normalised_assigned*players_winnings_normalised[np.newaxis,:]/total_tokens_assigned[np.newaxis,:],
            axis = 1
        )
    ...
    # Calculate the total wininngs from each alpha
    total_tokens_assigned = np.sum(total_normalised_assigned*optimal_pdf[:,np.newaxis], axis=0)
    total_winnings = np.sum(
        total_normalised_assigned*players_winnings_normalised[np.newaxis,:]/total_tokens_assigned[np.newaxis,:],
        axis = 1
    )
