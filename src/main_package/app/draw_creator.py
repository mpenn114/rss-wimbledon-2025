...
def display_draw_creator():
    ...
    with col1:
        if st.button('Add Match'):
            if name1 in st.session_state[names_cache] or name2 in st.session_state[names_cache]:
    ...
    with col3:
        ...
        names_dataframe['Match'] = np.cumsum((np.arange(len(names_dataframe)).astype(int)+1)%2)
        st.table(names_dataframe)

    if st.button('Save'):
        saved_dataframe = names_dataframe[['Player']]
        saved_dataframe.columns = ['player_name']
        saved_dataframe.to_csv(f"src/main_package/data/wimbledon_draw_{'male' if male_data else 'female'}.csv", index=False)
