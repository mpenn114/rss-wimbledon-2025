...
class EloModel(BaseAlternativeModel):
    ...
    def __init__(
        ...
        rating_decay: float = 0.0,
        surface_adjustment: bool = True,
    ):
        """
        ...
        if surface_adjustment:
            self.surface_ratings: Dict[
                Tuple[str, str], float
            ] = {}  # (player, surface) -> rating

    def _get_rating(
        self, player: str, surface: str = None, current_date: datetime = None
    ) -> float:
        """
            ...
            if self.surface_adjustment and surface:
                ...
                if key in self.surface_ratings:
                    self.surface_ratings[key] = max(
                        self.initial_rating, self.surface_ratings[key] - decay_amount
                    )
            else:
                if player in self.player_ratings:
                    self.player_ratings[player] = max(
                        self.initial_rating, self.player_ratings[player] - decay_amount
                    )
    ...
    def get_top_players(self, n: int = 10) -> list:
        ...
        sorted_players = sorted(
            self.player_ratings.items(), key=lambda x: x[1], reverse=True
        )
