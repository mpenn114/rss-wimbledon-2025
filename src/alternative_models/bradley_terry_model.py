...
class BradleyTerryModel(BaseAlternativeModel):
    ...
    def _estimate_strengths(self, match_data: pd.DataFrame) -> Dict[str, float]:
        ...
        # Get unique players
        players = list(
            set(match_data["Winner"].unique()) | set(match_data["Loser"].unique())
        )
        ...
        winner_indices = np.array(
            [player_to_idx[winner] for winner in match_data["Winner"]]
        )
        loser_indices = np.array(
            [player_to_idx[loser] for loser in match_data["Loser"]]
        )
        ...
        def negative_log_likelihood(log_strengths):
            ...
            prob_win = strengths[winner_indices] / (
                strengths[winner_indices] + strengths[loser_indices]
            )
        ...
        strength_dict = {
            player: optimized_strengths[idx] for player, idx in player_to_idx.items()
        }
    ...
    def predict(self, target_tournament: str, target_year: int, male_data: bool):
        ...
        training_cutoff = tournament_start - pd.Timedelta(
