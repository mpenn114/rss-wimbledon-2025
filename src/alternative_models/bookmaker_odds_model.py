...
class BookmakerOddsModel(BaseAlternativeModel):
    ...
    def predict(self, target_tournament: str, target_year: int, male_data: bool,
        ...
        target_matches['winner_strength'] = target_matches['Winner'].map(player_strength_map)
        target_matches['loser_strength'] = target_matches['Loser'].map(player_strength_map)

        # Calculate the max (worst) player and fill NA values
        max_strength = np.max(target_matches[~pd.isna(target_matches['loser_strength'])]['loser_strength'])
        target_matches[['winner_strength','loser_strength']]=target_matches[['winner_strength','loser_strength']].fillna(max_strength)

        # Calculate the model probability of the winner winning
        target_matches['predicted_prob_winner'] = 1/(1+np.exp(target_matches['winner_strength'] - target_matches['loser_strength']))
        
        # Save the predictions
        self._save_predictions(target_matches[['Winner','Loser','predicted_prob_winner']].reset_index(drop=True),
                               target_tournament,target_year,male_data)
