...
class CatBoostTennisModel(BaseAlternativeModel):
    ...
    def _calculate_player_statistics(
        self, data: pd.DataFrame, player_name: str, cutoff_date: pd.Timestamp
    ) -> Dict:
        """
        ...
        recent_data = data[
            (data["match_date"] >= twelve_months_ago)
            & (data["match_date"] < cutoff_date)
        ].copy()
        ...
        if total_matches > 0:
            ...
            stats["points_pct_12m"] = (
                points_won / total_possible_points if total_possible_points > 0 else 0.0
            )
    ...
    def _extract_features(
        ...
        player1_stats = self._calculate_player_statistics(
            all_data, player1_name, match_date
        )
        player2_stats = self._calculate_player_statistics(
            all_data, player2_name, match_date
        )

        features["player1_points_pct_12m"] = player1_stats["points_pct_12m"]
        features["player2_points_pct_12m"] = player2_stats["points_pct_12m"]
        features["player1_avg_rank_winner_12m"] = player1_stats[
            "avg_rank_when_winner_12m"
        ]
        features["player2_avg_rank_winner_12m"] = player2_stats[
            "avg_rank_when_winner_12m"
        ]
        features["player1_avg_rank_loser_12m"] = player1_stats[
            "avg_rank_when_loser_12m"
        ]
        features["player2_avg_rank_loser_12m"] = player2_stats[
            "avg_rank_when_loser_12m"
        ]
    ...
    def _prepare_training_data(self, training_data: pd.DataFrame) -> tuple:
        ...
        for idx, row in training_data.iterrows():
            ...
            features = self._extract_features(
                row, player1_is_winner, training_data, match_date
            )
    ...
    def predict(
        ...
        print(
            f"Training period: {training_data['match_date'].min().date()} to {tournament_start.date()}"
        )
        ...
        self.model.fit(
            X_train[~val_filter],
            y_train[~val_filter],
            eval_set=(X_train[val_filter], y_train[val_filter]),
            use_best_model=True,
        )
        ...
        for _, match in target_matches.iterrows():
            ...
            features = self._extract_features(
                match, player1_is_winner=True, all_data=all_data, match_date=match_date
            )
        ...
        y_true_full = np.concatenate(
            [np.ones(len(prediction_df)), np.zeros(len(prediction_df))]
        )
