...
class XGBoostTennisModel(BaseAlternativeModel):
    """
    Simple baseline XGBoost model for tennis match prediction.
    ...
    def __init__(self, xgb_params: Optional[Dict] = None):
        """
        Initialize the XGBoost Tennis Model.
    ...
    def _extract_features(self, row: pd.Series, player1_is_winner: bool) -> Dict:
        """
        ...
        # Ranking difference (negative = player1 ranked better)
        features['rank_diff'] = features['player1_rank'] - features['player2_rank']
        
        # Points features
        w_pts = row.get('WPts', 0)
        l_pts = row.get('LPts', 0)
        
        if player1_is_winner:
            features['player1_pts'] = w_pts if pd.notna(w_pts) else 0
            features['player2_pts'] = l_pts if pd.notna(l_pts) else 0
        else:
            features['player1_pts'] = l_pts if pd.notna(l_pts) else 0
            features['player2_pts'] = w_pts if pd.notna(w_pts) else 0
        
        # Points difference
        features['pts_diff'] = features['player1_pts'] - features['player2_pts']
    ...
    def _prepare_training_data(self, training_data: pd.DataFrame) -> tuple:
        ...
        for idx, row in training_data.iterrows():
            ...
            features = self._extract_features(row, player1_is_winner)
    ...
    def predict(self, target_tournament: str, target_year: int, male_data: bool):
        ...
        print(f"Training XGBoost model on {len(training_data)} matches...")
        ...
        y_true_full = np.concatenate([np.ones(len(prediction_df)), np.zeros(len(prediction_df))])
