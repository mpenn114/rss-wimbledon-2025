...
def run_token_pipeline(
    ...
    parameters = ModelParameters(
        temporal_decay=0.05,
        clay_weight=clay_weight_male,
        grass_weight=grass_weight_male,
    )

    # Calculate the estimated prize money
    male_prize_money = predict_tournament_prize_money(
        True,
        parameters,
        tournament,
        tournament_year,
        strength_forecast_only=calculate_strengths_only,
    )

    # Define the parameters
    parameters = ModelParameters(
        temporal_decay=0.05,
        clay_weight=clay_weight_female,
        grass_weight=grass_weight_female,
    )
    female_prize_money = predict_tournament_prize_money(
        False,
        parameters,
        tournament,
        tournament_year,
    ...
    combined_prize_money.to_csv(
        f"results/combined_prize_money_{tournament}_{tournament_year}.csv", index=False
    )
